...
//    }
    ...
    //qDebug() << "3. Parse each of the bits in it ";
    //if (progress)
    //{
...
//         qDebug() << "NRPN " << QString::fromStdString(title);
//        delete _nrpn;
//          qDebug() << "NRPN " << QString::fromStdString(title);
...
//        qDebug() << "NRPN " << QString::fromStdString(title);
    ...
    else if (line.substr(0,4) == "Key[")
    {
        ...
        //qDebug() << QString::fromStdString(bank_s) << "Je suis domi";
        int a = line.find(',')+1;
        std::string patch_s(line.substr(a, line.find(']')-a));
        //qDebug() << QString::fromStdString(patch_s) << "Je suis domi";
        int bank = -1, patch = -1;
        if (bank_s != "*")
        {
...
{
    for (int n = 0; n < 128; ++n) _names[n] = NULL;
        ...
        if (line == matchstr)
        {
                   ...
                   && (line.size() == 0 || (line[0] != '.' && line[0] != '[')))
            {
                ...
                if (line.find('=') != line.npos)
                {
                    ...
                    //qDebug() << "Before" << QString::fromStdString(line) << no;
                    delete _names[no];
    ...
    ()
{
    if (!searched_yet)
    {
        ...
        if (!in.good())
        {
            qDebug() << "Cannot open instrument definition file";
            return ins;
        }
        //if (progress)
        //{
...
//            progressCount++;
        ...
        while (!in.eof())
        {
            ...
            if (line.size() && line[0] == '[')
            {
                ...
                //qDebug() << QString::fromStdString(line.substr(1,line.size()-2));
            }
//            if (progress && !(progressCount%20))
//            {
//                progress->progress(in.tellg());
...
{
    return new Instrument(title, filename);
}
